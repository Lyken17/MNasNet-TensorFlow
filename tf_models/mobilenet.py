...
@slim.add_arg_scope
def mobilenet_base(
		inputs, conv_defs, multiplier=1.0, final_endpoint=None, output_stride=None, use_explicit_padding=False,
	...
	with _scope_all(scope, default_scope='Mobilenet'), \
		...
		for i, opdef in enumerate(conv_defs['spec']):
			...
			try:
				...
				print("Register layer with params: ", params)
			except Exception:
...
@slim.add_arg_scope
def mobilenet(inputs, num_classes=1001, prediction_fn=slim.softmax, reuse=None, scope='Mobilenet', base_only=False,
              **mobilenet_args):
	...
	with tf.variable_scope(scope, 'Mobilenet', reuse=reuse) as scope:
		...
		print(mobilenet_args["conv_defs"]["spec"][0])
...
def training_scope(is_training=True, weight_decay=0.00004, stddev=0.09, dropout_keep_prob=0.8, bn_decay=0.997):
	...
	with slim.arg_scope([slim.conv2d, slim.fully_connected, slim.separable_conv2d],
			weights_initializer=weight_intitializer, normalizer_fn=slim.batch_norm), \
